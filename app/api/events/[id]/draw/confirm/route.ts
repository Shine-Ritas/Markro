...
import { confirmDrawSession, getActiveDrawSession } from "@/services/draw.service";
...
export async function POST(_request: Request, context: RouteContext) {
  try {
    ...
    const result = await confirmDrawSession(
    ...
    log.error({ err: error }, "[draw confirm]");
    return NextResponse.json({ error: "Failed to confirm draw" }, { status: 500 });
  }
}
